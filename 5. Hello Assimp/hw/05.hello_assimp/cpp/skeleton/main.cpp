...
void compose_imgui_frame()
{
  ...
  // control window
  {
    ...
    ImGui::RadioButton("Camera 2", &g_cam_select_idx, 2);
...
void render_scene()
{
  ...
  //std::cout << g_obj_select_idx << '\n';
  for (std::size_t i = 0; i < g_objects.size(); ++i)
  {
    // TODO : draw each object
    mat_model = g_objects[i].get_model_matrix();
    mat_PVM = mat_proj * mat_model * mat_view;
